...
export async function sc900CloudFixture(workspace: string, scoped = false) {
  ...
  const staticPlan = buildSc900StaticPlan(publicationInput, scoped ? sc900ScopedReview(prepared) : sc900BankReview(prepared));
  ...
  const final = await writeSc900FinalApproval(staticPlan, {
    ...
    ...(prepared.manifest.discussionScope ? { discussionScope: prepared.manifest.discussionScope } : {}),
  }, workspace);
  ...
  const approval = CloudApplyApprovalSchema.parse({
    ...
    ...(plan.discussionScope ? { discussionScope: plan.discussionScope } : {}),
  });
