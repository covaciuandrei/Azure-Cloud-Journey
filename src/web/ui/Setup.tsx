...
export function Setup({ mode, catalog, onStart, initialTopics }: {
  ...
  const [topics, setTopics] = useState<TopicId[]>(() => initialTopics ? [...initialTopics] : [...topicIdsForExam(examId)]);
  ...
  return <section className="setup-page">
    <div className="page-heading"><div><h1>{mode === "exam" ? "Practice exam" : "Practice"}</h1>
      <p>{mode === "exam" ? `40 questions in ${exam.mockDurationMinutes} minutes. Answers appear after you finish.` :
    ...
    <form className="setup-form" onSubmit={(event) => {
      ...
      {mode === "free" ? <fieldset className="setup-options"><legend>Number of questions</legend>
        ...
        <div><dt>Time limit</dt><dd>{exam.mockDurationMinutes} minutes</dd></div><div><dt>Feedback</dt><dd>After completion</dd></div></dl>}
      ...
      <TopicFilter questions={pool} selected={topics} onChange={setTopics} examId={examId} />
      ...
      {mode === "exam" && <p className="muted">The timer continues if you leave or reload. Unanswered questions remain unmarked.
        This practice format is not the official exam scoring system or a claim about the real exam's exact question count.</p>}
